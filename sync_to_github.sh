...
# 脚本名称: sync_to_github.sh
# 功能: 增量同步 GitLab 的变更到 GitHub（只推送有变化的分支）
...
echo -e "${GREEN}========================================${NC}"
echo -e "${GREEN}  GitLab → GitHub 增量同步工具${NC}"
echo -e "${GREEN}========================================${NC}"
echo ""

# 1. 检查是否已添加 github remote
if ! git remote | grep -q "github"; then
    echo -e "${YELLOW}添加 GitHub remote...${NC}"
    git remote add github git@github.com:PeiJueChen/pgyer-enhance-private.git
    echo -e "${GREEN}✓ GitHub remote 已添加${NC}"
else
    echo -e "${GREEN}✓ GitHub remote 已存在${NC}"
fi

echo ""

# 2. 更新远程信息
echo -e "${BLUE}步骤 1: 从 origin 拉取最新数据...${NC}"
...
while IFS= read -r branch; do
    ...
    if git ls-remote --heads github "$branch" | grep -q "$branch"; then
        ...
        REMOTE_COMMIT=$(git ls-remote --heads github "$branch" | awk '{print $1}')
...
if [ "$NEED_PUSH_BRANCHES" = true ]; then
    read -p "$(echo -e ${YELLOW}是否推送这 $TOTAL_TO_PUSH 个分支到 GitHub? (y/n): ${NC})" -n 1 -r
    echo
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        PUSH_BRANCHES=true
    else
...
if [ "$PUSH_BRANCHES" = true ]; then
    ...
    for branch in "${NEW_BRANCHES[@]}"; do
        echo -e "  推送新分支: $branch"
        if git push github "$branch" > /dev/null 2>&1; then
            PUSHED=$((PUSHED + 1))
        else
...
NEW_TAGS=()
EXISTING_TAGS_ON_GITHUB=$(git ls-remote --tags github | awk '{print $2}' | sed 's/refs\/tags\///' | sort)

for tag in $(git tag); do
    if ! echo "$EXISTING_TAGS_ON_GITHUB" | grep -q "^${tag}$"; then
        NEW_TAGS+=("$tag")
    fi
done
...
if [ $TOTAL_NEW_TAGS -gt 0 ]; then
    ...
    read -p "$(echo -e ${YELLOW}是否推送这些新标签到 GitHub? (y/n): ${NC})" -n 1 -r
    echo
    if [[ $REPLY =~ ^[Yy]$ ]]; then
